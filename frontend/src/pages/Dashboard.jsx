...
export default function Dashboard() {
  ...
  const [mode, setMode]           = useState("grounded"); // grounded | extended
  ...
  const loadSessions = async () => {
    try {
      ...
      if ((data.data || data).length > 0 && !activeSession) openSession((data.data || data)[0]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setSessions(p => p.map(s => s._id === activeSession._id ? { ...s, title: payload.sessionTitle || s.title } : s));
    } catch (e) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const payload = data.data || data;
      setDocuments(p => [...p, ...(payload.documents || [payload])]);
    } catch (e) { console.error(e); }
              ...
              <div className="chat-input-wrap">
                <div className="chat-input-box">
                  <textarea
                    ...
                    placeholder={mode === "grounded"
                      ...
                      : "Ask anything — I'll combine your notes with broader knowledge…"
                    }
                    rows={1}
                    disabled={sending}
                  />
                ...
                <p className="chat-input-hint">
                  {mode === "grounded"
                    ...
                    : "🌐 Extended — uses your notes and expands with broader knowledge"
                  }
                </p>
              </div>
            </>
